...
execute if score #timer_game lobby.tc.data matches 41.. as @a[scores={uhc.players.lang=061801}] unless entity @s[tag=!mgs.tc.player,tag=!mgs.tc.spec] run tellraw @s [{"text":"Un joueur vient de terminer le mini-jeu, temps avant fin réduit à 40 secondes","color":"#FF3F3F"}]
execute if score #timer_game lobby.tc.data matches 41.. as @a[scores={uhc.players.lang=051407}] unless entity @s[tag=!mgs.tc.player,tag=!mgs.tc.spec] run tellraw @s [{"text":"A player has just finished the mini-game, time to end reduced to 40 seconds","color":"#FF3F3F"}]
