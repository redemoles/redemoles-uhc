...
scoreboard players set #total_first_$(namespace)_$(line)_$(column) bhc.data 1
...
$execute as @p[tag=!bhc.new_adv,advancements={$(namespace):$(line)_$(column)=true}] run scoreboard players set #total_first_$(namespace)_$(line)_$(column) bhc.data 0
# Ligne
...
$execute if entity @s[tag=bhc.new_adv] if score #stepa_enabled bhc.data matches $(step) run function bhc:scenarios/08/advancements/stepa
$execute if entity @s[tag=bhc.new_adv] if score #stepb_enabled bhc.data matches $(step) run function bhc:scenarios/08/advancements/stepb

## Si FFA → Don du dernier succès non complété d'une ligne/colonne
$execute if score #TeamSize uhc.data.setup matches 1 if score @s bhc.line_$(line) = #line-1 bhc.data run function bhc:scenarios/08/advancements/ffa_line with storage $(namespace) $(line)_$(column)
$execute if score #TeamSize uhc.data.setup matches 1 if score @s bhc.column_$(column) = #column-1 bhc.data run function bhc:scenarios/08/advancements/ffa_column with storage $(namespace) $(line)_$(column)
