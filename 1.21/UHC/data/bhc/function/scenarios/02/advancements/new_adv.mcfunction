...
$execute if score #team_first_$(line) bhc.data matches 1 run function bhc:scenarios/02/advancements/message_line
$execute if score #team_first_$(column) bhc.data matches 1 run function bhc:scenarios/02/advancements/message_column
...
$execute if entity @s[tag=bhc.new_adv] if score #stepa_enabled bhc.data matches $(step) run function bhc:scenarios/02/advancements/stepa with storage $(namespace) $(line)_$(column)
$execute if entity @s[tag=bhc.new_adv] if score #stepb_enabled bhc.data matches $(step) run function bhc:scenarios/02/advancements/stepb with storage $(namespace) $(line)_$(column)

## Si FFA → Don du dernier succès non complété d'une ligne/colonne
$execute if score #TeamSize uhc.data.setup matches 1 if score @s bhc.line_$(line) = #line-1 bhc.data run function bhc:scenarios/02/advancements/ffa_line with storage $(namespace) $(line)_$(column)
$execute if score #TeamSize uhc.data.setup matches 1 if score @s bhc.column_$(column) = #column-1 bhc.data run function bhc:scenarios/02/advancements/ffa_column with storage $(namespace) $(line)_$(column)
