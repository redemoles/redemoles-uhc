...
execute store result score #temp uhc.data.setup if entity @a[scores={uhc.id.teams=09}]
...
execute if score #anonyme_team uhc.data.setup matches 0 run team join 09 @s
execute if score #anonyme_team uhc.data.setup matches 1 run team join 91 @s

tellraw @s[scores={uhc.players.lang=061801}] [{"text":"Tu as rejoint l'","color":"#3FE7FF","bold":false},{"text":"Équipe Rouge c.","color":"red","bold":false}]
tellraw @s[scores={uhc.players.lang=051407}] [{"text":"You joined the ","color":"#3FE7FF","bold":false},{"text":"L. Red Team","color":"red","bold":false}]
