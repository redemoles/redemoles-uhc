...
execute if score #nzl uhc.gamemode matches 1 if score #type_start nzl.data matches 0 if score #gamemode uhc.host.menu matches 1 unless items entity @s inventory.11 *[minecraft:item_name='{"text":"Évolution au choix","color":"#3FFFFF","italic":false}'] run function uhc:pre_game/menu/load/gamemode/nzl/type
execute if score #nzl uhc.gamemode matches 1 if score #type_start nzl.data matches 1 if score #gamemode uhc.host.menu matches 1 unless items entity @s inventory.11 *[minecraft:item_name='{"text":"Random Évolution au Start","color":"#FF3F3F","italic":false}'] run function uhc:pre_game/menu/load/gamemode/nzl/type
