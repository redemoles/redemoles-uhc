...
execute unless entity @s[scores={uhc.id.teams=91}] unless items entity @s[tag=uhc.random_team.in_a_team,scores={uhc.players.lang=1}] inventory.12 *[minecraft:custom_data={Tags:"team_91"}] run tellraw @s [{"text":"Tu ne peux pas quitter ton équipe car elles ont déjà été générées.","color":"#FF3F3F","bold":false}]
execute unless entity @s[scores={uhc.id.teams=91}] unless items entity @s[tag=uhc.random_team.in_a_team,scores={uhc.players.lang=2}] inventory.12 *[minecraft:custom_data={Tags:"team_91"}] run tellraw @s [{"text":"You can't leave your team because they've already been generated.","color":"#FF3F3F","bold":false}]
...
execute as @s[tag=uhc.host,tag=host.menu.teams] run function uhc:pre_game/menu/load/teams/gamemode/vanilla_random/
execute as @s[tag=!uhc.host,tag=menu.player.teams] run function uhc:pre_game/menu/load/teams/gamemode/vanilla_random/
