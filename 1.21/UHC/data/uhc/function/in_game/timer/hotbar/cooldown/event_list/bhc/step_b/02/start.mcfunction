...
execute if score #stepb_start bhc.data.temp = #min_cooldown_temp uhc.data.temp run data modify storage uhc:temp hotbar.alert_fra append value [{"text":"Début de l'","color":"#FFFFFF"}, {"text":"Ère Viking","color":"#CF6F3F"}]
execute if score #stepb_start bhc.data.temp = #min_cooldown_temp uhc.data.temp run data modify storage uhc:temp hotbar.alert_eng append value [{"text":"Start of the ","color":"#FFFFFF"}, {"text":"Viking Era","color":"#CF6F3F"}]
