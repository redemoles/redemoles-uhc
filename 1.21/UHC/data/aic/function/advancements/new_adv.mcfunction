...
$execute if score #first_time aic.data.score matches 1 run tellraw @a[predicate=uhc:id_teams,scores={uhc.players.lang=2}] [{"text":""},{"selector":"@s","bold":true},{"text":" just got the item: "},{"text":"$(item_name)","color":"#57FF3F","hover_event":{"action": "show_text","value":[{"text":"$(description)"}]}},{"text":" ("},{"text":"$(collection_name)","color":"#3FE7FF"},{"text":")"}]
