...
with open(DESTINATION, "w") as f:
	...
	for x in range(-ZONE,ZONE,FORCELOAD_ZONE):
		...
		for y in range(-ZONE,ZONE,FORCELOAD_ZONE):
			...
			# Forceload Add
			f.write(f"execute if score #chunk_pregen worldpregen matches {i} run forceload add {x} {y} {upperX} {upperY}\n")
			
			# Forceload Remove
			f.write(f"execute if score #chunk_pregen worldpregen matches {i+FORCELOAD_DELAY} run forceload remove {x} {y} {upperX} {upperY}\n")
	...
	for i in range(0, NB_MESSAGES+1):
		...
		size = (ZONE * 2)
		f.write(f'execute if score #chunk_pregen worldpregen matches {tick} run tellraw @a[scores={{worldpregen=1..}}] [{{"text":"Overworld : {size}x{size} ({i}/{NB_MESSAGES}, {percentage}%)"}}]\n')
